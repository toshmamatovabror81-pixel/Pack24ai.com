...
export async function generateSupervisorReport(
    ...
    return {
        totalRequests,
        completedRequests,
        collectionWeight: collections.reduce((sum, row) => sum + Number(row.actualWeight), 0),
        collectionAmount: collections.reduce((sum, row) => sum + Number(row.totalAmount), 0),
        activeDrivers,
        intakeWeight: intakeLogs.reduce((sum, row) => sum + Number(row.weightKg), 0),
        intakeAmount: intakeLogs.reduce((sum, row) => sum + Number(row.totalAmount), 0),
        pressedKg: pressLogs.reduce((sum, row) => sum + Number(row.pressedKg), 0),
        baleCount: pressLogs.reduce((sum, row) => sum + Number(row.baleCount), 0),
        expenseAmount: expenseLogs.reduce((sum, row) => sum + Number(row.expenseAmount), 0),
        advanceAmount: expenseLogs.reduce((sum, row) => sum + Number(row.advanceAmount), 0),
        soldWeight: salesLogs.reduce((sum, row) => sum + Number(row.weightKg), 0),
        soldAmount: salesLogs.reduce((sum, row) => sum + Number(row.totalAmount), 0),
        soldBaleCount: salesLogs.reduce((sum, row) => sum + Number(row.baleCount), 0),
    };
}
